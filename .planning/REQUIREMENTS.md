...
- [ ] **SAFE-01**: A commit that changes only `_sass/**` triggers a production deploy. Today `deploy.yml`'s push path filter watches `assets/**, **.bib, **.html, **.js, **.liquid, **/*.md, **.yml, Gemfile*` — `_sass/_custom.scss` matches none of them, so the entire design pass would silently never reach the live site, with no failed workflow to signal it.
- [ ] **SAFE-02**: A production deploy leaves `phamhakhanhchi.com` resolving. `CNAME` must appear in `_site/` and survive `JamesIves/github-pages-deploy-action@v4` replacing the `gh-pages` branch. Verified, not assumed — the two deploys on 2026-09-10 both shipped without it and the domain was rescued by a manual commit onto `gh-pages`; `CNAME` reached `main` only afterwards and no deploy has exercised it since.
